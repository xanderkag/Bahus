...
**CRITICAL:** Vercel (Frontend) and Railway (Backend) are configured for automatic Continuous Deployment via GitHub. 
Whenever you make changes to the Bahus codebase, you **MUST** run the following commands to trigger the build:
1. `git add src/` (or specific files)
2. `git commit -m "feat/fix: ..."`
3. `git push`
Do not say "Запушил на Railway" directly. Use "Запушил в Git (автоматически собирается в Vercel/Railway)", otherwise the user gets confused.

---

## 📅 Session Log: 2026-04-21 (Evening)

> **Фокус сессии: Формирование КП, Экстремальная Производительность Поиска и UI Полировка.** Доведены до идеала клиентские таблицы и поиск.

### ⚡ Линия 1: Токенизированный поиск (WeakMap Caching)
- Полностью переписан движок локального поиска для таблиц Импортов, КП и Позиций.
- Внедрён **токенизированный поиск**: запрос разбивается на слова, и проверяется независимое наличие каждого токена (например, "вино красное" найдет "красное вино").
- Внедрено **`WeakMap` кэширование**: поисковый индекс (склеивающий название, поставщика, страну, категорию и т.д.) собирается ровно один раз для каждого объекта и автоматически инвалидируется при редактировании ячейки (так как иммутабельный стейт меняет ссылку на объект). Скорость поиска стала моментальной даже на тысячах строк.

### 💅 Линия 2: Полировка таблиц и UX
- Добавлены полноценные фильтры и сортировки для колонок: `Поставщик`, `Тип документа`, `Актуальность` и `Артикул`. Все выпадающие списки генерируются динамически.
- Реализованы всплывающие уведомления об успешном добавлении в общую базу (`Success Modal` с авто-закрытием через 1.5 секунды) вместо технических алертов.
- Вычищен UI модалки деталей позиции (убраны placeholder-ы ручного матчинга, если он не проводился).
- Исправлен Data Synchronization баг: ранее позиции пропадали из общей вкладки из-за хардкодного фильтра по `owner` (`manager@bahus`). Теперь стейт `getProductsByScope` корректно отображает всю загруженную номенклатуру.

### 💰 Линия 3: Формирование Коммерческих Предложений (КП)
- Проведен технический аудит конвейера КП. Подтверждена работоспособность флоу: выбор позиций -> редактирование черновика (цены, скидки, кол-во) -> расчет маржи на лету -> экспорт в клиентский Excel-файл без технических полей.

---

## 📅 Session Log: 2026-04-20 & Early 21

> **Фокус сессии: Стабилизация, Обогащение фото и Полный Редизайн деталей позиции.** Мы уперлись в квоты деплойментов Vercel/Railway, поэтому проект заморожен до обновления лимитов хостинга. Вся архитектура зафиксирована.

### 🧱 Линия 1: Инфраструктура и Стабилизация
- **PyMuPDF Memory Protection:** Добавлено ограничение в `max_pages=10` и снижение `dpi=150` для Vision режима извлечения PDF. Это предотвратило падения (OOM - Out of Memory) бэкенда на Railway при обработке тяжелых векторных прайсов.
- **Database Resilience:** Добавлен `connect_timeout=3` для коннектов к пулеру Supabase psycopg, чтобы Railway не зависал навечно при проблемах сети.

### 🖼 Линия 2: Автоматическое обогащение фотографиями (Serper API)
- Разработан эндпоинт `POST /api/rows/{row_id}/enrich-photo` в `postgres_api.py`.
- Бэкенд склеивает `normalized_name` + "wine bottle white background" и через **Serper.dev** API вытаскивает прямой `image_url` с Google Images.
- Добавлена схема `ALTER TABLE import_row ADD COLUMN image_url` и сохранение на уровне базы.
- Имплементирована кнопка **"🔍 Найти в сети"**, встроенная прямо во фронтенд для автоматической подгрузки фото без ручного поиска.

### 💅 Линия 3: Радикальный UI Рефакторинг (Окно деталей)
- Вычищен "технический трэш" из окна `row-detail-dialog`.
- **Light Theme Fix:** Убраны жесткие черные градиенты на плейсхолдерах фотографий, заменены на премиальный светлый Glassmorphism.
- **Матрица карточек → Чистый список:** Десять микро-карточек превращены в один элегантный список параметров (Key-Value) с тонкими разделителями (Apple style). Убраны громоздкие технические ID и логи ошибок, которые не нужны менеджерам по закупкам.

### 📚 Линия 4: Документация и Техдолг
- Создан и закоммичен `docs/TECH_DEBT.md`. Расписаны проблемы и стратегии решения для двух будущих задач:
  1. Выкачивание каталогов напрямую с WineStyle (проблема Cloudflare) и Wine-Shopper (поиск по URL).
  2. Разбор сложных "Адских" Excel-прайсов с вшитыми картинками (стратегии распаковки `openpyxl` или конвертация в PDF для Vision API).

...
function renderImportsTable(state) {
  ...
  const confirmStats = getImportConfirmStats(state);
  return getVisibleImports(state)
    .map((item, index) => {
      ...
      const cs = confirmStats[item.id] || { checked: 0, total: 0 };
      const confirmPct = cs.total > 0 ? Math.round((cs.checked / cs.total) * 100) : 0;
      const allDone = cs.total > 0 && cs.checked === cs.total;
      return `
        <tr class="${selectedImportId === item.id ? "is-active" : ""} clickable-row" data-action="selectImport" data-import-id="${item.id}" style="cursor:pointer">
          ...
          <td>
            ${cs.total > 0
              ? `<div class="import-confirm-progress" title="Подтверждено: ${cs.checked} из ${cs.total}">
                  <div class="import-confirm-bar">
                    <div class="import-confirm-fill${allDone ? ' import-confirm-fill-done' : ''}" style="width:${confirmPct}%"></div>
                  </div>
                  <span class="import-confirm-label">${cs.checked}/${cs.total}</span>
                </div>`
              : `<span style="color:var(--text-3,#888);">—</span>`
            }
          </td>
        ...
        <article class="panel overview-files-panel" style="min-width: 0; display: flex; flex-direction: column;">
          <div class="panel-header overview-panel-header">
            ...
            <div class="toolbar-actions overview-table-actions">
              <button class="toolbar-btn" data-action="openUploadFilesModal" title="Загрузить прайс-лист поставщика">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>
                <span>Загрузить</span>
              </button>
              <button class="toolbar-btn" data-action="dispatchSelectedImport" ${!state.ui.selectedImportId ? "disabled" : ""} title="Отправить выбранный файл на обработку ИИ">
              ...
              <button class="toolbar-btn toolbar-btn-danger" data-action="promptDeleteImport" ${!state.ui.selectedImportId ? "disabled" : ""} title="Удалить выбранный прайс">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
                <span>Удалить</span>
              </button>
              <button class="toolbar-btn" data-action="openExportModal" title="Экспорт списка импортов">
      ...
      <article class="panel overview-products-panel">
        ...
        <div class="table-wrap overview-table-wrap">
          <div class="overview-table-toolbar">
            <div class="toolbar-actions overview-table-actions">
              <button class="toolbar-btn" data-action="openTableSettings" data-table="overview" title="Настроить видимость и порядок столбцов">
              ...
              ${(() => {
                const isProcessing = currentImport && ["queued", "pending", "processing"].includes(currentImport.status);
                const isSelectionEmpty = selectedCount === 0;
                const isActionsDisabled = isProcessing || isSelectionEmpty;
                return `
                  <button class="toolbar-btn" data-action="selectAllVisibleRows" ${isProcessing ? "disabled" : ""} title="Выделить все строки после текущей фильтрации">
                  ...
                  <button class="toolbar-btn" data-action="clearSelectedRows" ${isSelectionEmpty ? "disabled" : ""} title="Снять текущее выделение">
                  ...
                  <div class="toolbar-divider"></div>
                  <button class="toolbar-btn toolbar-btn-accent" data-action="markSelectedChecked" ${isActionsDisabled ? "disabled" : ""} title="Подтвердить выделенные позиции">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    <span>Подтвердить</span>
                  </button>
                  <button class="toolbar-btn toolbar-btn-danger" data-action="excludeSelectedRows" ${isActionsDisabled ? "disabled" : ""} title="Исключить выделенные строки из дальнейшей обработки">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
                    <span>Исключить</span>
                  </button>
                  <button class="toolbar-btn" data-action="openDetailsForSelectedRow" ${selectedCount !== 1 ? "disabled" : ""} title="Подробная информация о позиции">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/></svg>
                    <span>Детали</span>
                  </button>
                  <button class="toolbar-btn toolbar-btn-primary" data-action="addSelectionToQuote" ${isActionsDisabled ? "disabled" : ""} title="Добавить выделенные строки в состав КП">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                    <span>В&nbsp;КП</span>
                  </button>
                  <button class="toolbar-btn toolbar-btn-accent" data-action="buildQuote" ${isActionsDisabled ? "disabled" : ""} title="Сформировать рабочий сценарий КП по выделенным строкам">

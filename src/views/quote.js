...
function renderProcessingModal(state) {
  ...
  const title = state.ui.processingMessage?.title || "Подождите";
  const text = state.ui.processingMessage?.text || "Загрузка...";
  return `
    <div class="modal-overlay" style="z-index:9999;">
      <div class="app-dialog" style="width:min(400px,94vw);text-align:center;padding:40px 20px;">
        ...
        <p style="color:var(--muted);margin-top:8px;">${escapeHtml(text)}</p>
      </div>
    </div>

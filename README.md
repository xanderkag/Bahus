...
## Что реализовано (статус на 2026-04-30)

### ✅ Frontend (Vanilla JS)

- Загрузка файлов поставщиков через UI (drag-and-drop или выбор): PDF, Excel (.xlsx/.xls/.csv), изображения
- 4 типа документов: Прайс-лист, Нетто-прайс, Промо, Под запрос
...
В апреле 2026 мы мигрировали с простого PDF text extraction на OpenAI Assistants API, а затем на прямую Chat Completions API с двумя режимами:
- **Text mode**: PyMuPDF для PDF / openpyxl для Excel → текст → GPT-4o
- **Vision mode**: PyMuPDF render страниц в PNG → GPT-4o Vision (для сканов)
...
> ⚠️ **ПЕРЕД ДЕМО:** Код для PDF + Excel extraction написан и проверен синтаксически. Необходимо прогнать живой тест: загрузить один реальный файл с рабочим `OPENAI_API_KEY` и убедиться что позиции извлекаются. Тестовые файлы в `Bahus docs/`.
